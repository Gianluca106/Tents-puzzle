...
class BoardGameGui:
    ...
    def tick(self):
        ...
        for k, v in self._actions.items():
            if k in released and y < game.rows() and x < game.cols():
                game.play(x, y, v)
    ...
    def write(self, text, pos, cols=1):
        ...
        if cols == 1 and last in self._annots:
            ...
            g2d.draw_rect((x * W + 1, y * H + 1), (W - 2, H - 2))
            text = text[:-1]
...
def gui_play(game: BoardGame, actions=None, annots=None):
    if actions is None: actions = {"LeftButton": "", "RightButton": "flag"}
    if annots is None: annots = {"#": (GRAY, 0), "!": (GRAY, 2)}
    
    g2d.init_canvas((game.cols() * W, game.rows() * H + H))
    ui = BoardGameGui(game, actions, annots)
